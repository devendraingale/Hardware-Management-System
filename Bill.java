...
public class Bill extends JFrame implements ActionListener,ItemListener,MouseMotionListener,MouseListener

{
	JLabel l1,l2,l3,l4,l5,l6,l7,l8,l9,l10,l11,l12,l13,limg,l14;
	JTextField t1,t3,t4,t5,t6,t7,td1,td2,td3,td4,t8,td5;
...
//***********************************************************************
		limg=new JLabel(img);
		l1=new JLabel("Sales ID");
		l2=new JLabel("Customer Name");
		l3=new JLabel("Contact");
		l4=new JLabel("DATE");
		l5=new JLabel("Sales Oreder");
		...
		c3.setBounds(420,200,200,30); //product Comobox
		c1.setBounds(760,100,200,30);
...
//******************************************Auto increment BillId Code*****************************************
				...
				cn=DriverManager.getConnection("jdbc:odbc:MDII");
			    st=cn.createStatement();
				rs=st.executeQuery("Select * From SalesOrder");
				while (rs.next())
				{
				 ...
				 if(c==0)
				 {
					 try
					 {
			            ...
			            Statement st2=cn2.createStatement();
			             ResultSet rs10=st2.executeQuery("select * from SalesOrder");
			             while(rs10.next())
			                {
                          ...
                          bt++;
                         t=0;
						String str="insert into SalesOrder values("+ bt +",'"+cname+"','"+t3.getText()+"','"+t6.getText()+"','"+t4.getText()+"','"+pname[0]+"','"+pname[1]+"','"+pname[2]+"','"+pname[3]+"','"+pname[4]+"',"+qnt[0]+","+qnt[1]+","+qnt[2]+","+qnt[3]+","+qnt[4]+","+amt[0]+","+amt[1]+","+amt[2]+","+amt[3]+","+amt[4]+","+tamt[0]+","+tamt[1]+","+tamt[2]+","+tamt[3]+","+tamt[4]+","+t7.getText()+")";
	...
	if(b==b3)
	{
		try
		{
			...
			ResultSet rs1=st.executeQuery("Select * from SalesOrder where Bill_Id=" + id);
			c1.setVisible(false);
			t8.setVisible(true);
			rs1.next();
	...
	if(b==b4)
	{
      ...
      public void itemStateChanged(ItemEvent e)
		...
		//*******************Customer Name Choice*****************************
			           ...
			           rs=st.executeQuery("Select * from Customer where Customer_name='"+cname+"'");
			           while(rs.next())
			           {
						   con=rs.getString(6);
						   el=rs.getString(5);
...
//******************************ListBox Code**********************************
                       ...
                       li2.add(qnt1);
                       String str="select * from Product where Product_Name='"+ n + "'";
                       Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
				       cn=DriverManager.getConnection("jdbc:odbc:MDII");
				       st=cn.createStatement();
						...
						li3.add(rate);
						amt[t]=Integer.parseInt(rate);

					    int r=Integer.parseInt(rate);
					    int q=Integer.parseInt(qnt1);
					    int qtotal=r*q;
					    tamt[t]=qtotal;
					    String tot=Integer.toString(qtotal);
					    li4.add(tot);
                         t++;
                        ftotal=ftotal+qtotal;
                        t7.setText(" " + ftotal);
